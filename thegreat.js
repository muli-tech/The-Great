...
async function koala() {
  try {
    ...
    if (gnome && gnome.trim().toLowerCase() !== lund.toLowerCase()) {
      ...
      console.log(lawde); // Logs the decoded 'lawde' message
      process.exit(1); // Exit with an error code
    } else {
      // If the names match, log a success message and proceed
      console.log(`${endi}`); // Logs the decoded 'endi' message
      console.log(chalk.bgBlack(chalk.redBright('initializing ...'))); // Stylized log for initialization
    }
  } catch (error) {
